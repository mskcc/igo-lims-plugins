...
public class Covid19ElisaSampleImporter extends DefaultGenericPlugin {
    ...
    public Covid19ElisaSampleImporter() throws ServerException {
        setActionMenu(true);
    ...
    public PluginResult run() throws ServerException {
        try {
            ...
            List<DataRecord> requests = dataRecordManager.queryDataRecords("Request", "RequestId = '" + COVID_ELISA_REQUEST_ID + "'" , user);
    ...
    private List<Map<String, Object>> parseFileDataToSampleData(List<String> fileData, Map<String, Integer> headrValuesMap){
        ...
        for (int i = 1; i< fileData.size(); i++){
            ...
            if (rowValues.size()>0){
                ...
                String sampleId = COVID_ELISA_REQUEST_ID + "_" + nextSampleNumer;
    ...
    private Integer getNextSampleNumber() {
        ...
        try {
            List<DataRecord> samples = dataRecordManager.queryDataRecords("Sample", "RequestId = '" + COVID_ELISA_REQUEST_ID + "'", user);
            if (samples.size() > 0) {
    ...
    private boolean hasDuplicateSampleName(List<Map<String, Object>> parsedSampleData) throws IoError, RemoteException, NotFound, ServerException {
        List<DataRecord> samplesInRequest = dataRecordManager.queryDataRecords("Sample", "RequestId = '" + COVID_ELISA_REQUEST_ID + "' AND ExemplarSampleType = 'other'", user);
